...
enum  pkt_type {
    ...
    PKT_TYPE_SENSOR_SHT4x_TEMP = 0xB3, // float
    PKT_TYPE_SENSOR_SHT4x_HUMIDITY = 0xB4, // float

    PKT_TYPE_SENSOR_TVOC_INDEX = 0xB5, // float

    PKT_TYPE_SENSOR_TDS = 0xB6, //float
    PKT_TYPE_SENSOR_EC = 0xB7, //float
...
struct indicator_sensor_present_data
{
    ...
    struct sensor_present_data tds; //Add tds
    struct sensor_present_data ec;  //Add EC
...
struct indicator_sensor_history_data
{
    ...
    struct sensor_history_data tds; //Add tds
    struct sensor_history_data ec; //Add EC
...
static void __sensor_history_data_check(time_t now)
{
    ...
    __sensor_history_data_day_check(__g_sensor_history_data.tds.data_day,   "TDS", now - 3600);
    __sensor_history_data_week_check(__g_sensor_history_data.tds.data_week, "TDS",  now - 3600 * 24); //Add TDS

    __sensor_history_data_day_check(__g_sensor_history_data.ec.data_day,   "EC", now - 3600);
    __sensor_history_data_week_check(__g_sensor_history_data.ec.data_week, "EC",  now - 3600 * 24); //Add EC
...
static void __sensor_history_data_day_update(time_t now)
{
    ...
    __sensor_history_data_day_insert( __g_sensor_history_data.tds.data_day, &__g_sensor_present_data.tds, now); //Add TDS

    __sensor_history_data_day_insert( __g_sensor_history_data.ec.data_day, &__g_sensor_present_data.ec, now); //Add EC
...
static void __sensor_history_data_week_update(time_t now)
{
    ...
    __sensor_history_data_week_insert(__g_sensor_history_data.tds.data_week, &__g_sensor_present_data.tds, now); //Add TDS

    __sensor_history_data_week_insert(__g_sensor_history_data.ec.data_week, &__g_sensor_present_data.ec, now); //Add EC
...
/////////////////////////////TDS////////////////////////////////
            case PKT_TYPE_SENSOR_TDS: {
            struct view_data_sensor_data data;
            if( len < (sizeof(data.vaule) +1)) {
                break;
            }
    
            data.sensor_type = SENSOR_DATA_TDS;
            memcpy(&data.vaule, &p_data[1], sizeof(data.vaule));
            __sensor_present_data_update(&__g_sensor_present_data.tds, data.vaule);
...
/////////////////////////////EC////////////////////////////////
            case PKT_TYPE_SENSOR_EC: {
            struct view_data_sensor_data data;
            if( len < (sizeof(data.vaule) +1)) {
                break;
            }
    
            data.sensor_type = SENSOR_DATA_EC;
            memcpy(&data.vaule, &p_data[1], sizeof(data.vaule));
            __sensor_present_data_update(&__g_sensor_present_data.ec, data.vaule);
...
////////////////////////////EC END/////////////////////////////
        ...
        case PKT_TYPE_SENSOR_SHT4x_TEMP: {
            struct view_data_sensor_data data;
            if( len < (sizeof(data.vaule) +1)) {
                break;
            }
        ...
        case PKT_TYPE_SENSOR_SHT4x_HUMIDITY: {
            struct view_data_sensor_data data;
            if( len < (sizeof(data.vaule) +1)) {
                break;
            }
...
///////////////////////////////////TDS//////////////////////////////////////////////////
        ...
        case VIEW_EVENT_SENSOR_TDS_HISTORY: {
            ESP_LOGI(TAG, "event: VIEW_EVENT_SENSOR_TDS_HISTORY");
            struct view_data_sensor_history_data data;
            __sensor_history_data_get( &__g_sensor_history_data.tds,  &data);
            data.sensor_type = SENSOR_DATA_TDS;
            data.resolution  = 0;
            esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_SENSOR_DATA_HISTORY, &data, sizeof(struct view_data_sensor_history_data ), portMAX_DELAY);
            break;
...
///////////////////////////////////EC//////////////////////////////////////////////////
        ...
        case VIEW_EVENT_SENSOR_EC_HISTORY: {
            ESP_LOGI(TAG, "event: VIEW_EVENT_SENSOR_EC_HISTORY");
            struct view_data_sensor_history_data data;
            __sensor_history_data_get( &__g_sensor_history_data.ec,  &data);
            data.sensor_type = SENSOR_DATA_EC;
            data.resolution  = 0;
            esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_SENSOR_DATA_HISTORY, &data, sizeof(struct view_data_sensor_history_data ), portMAX_DELAY);
            break;
...
//debug ui 
        ...
        case VIEW_EVENT_SENSOR_HUMIDITY_HISTORY: {
            ...
            for(i = 0; i < 7; i++) { 
                ...
                data.data_week[i].timestamp =  time2 + i * 3600 * 24;;3600
                data.data_week[i].valid = true;
                if( min > data.data_week[i].min) {
                    min = data.data_week[i].min;
                }
...
//////////////////////////////////////////////TDS///////////////////////////////////////////////////////////////////////
    ...
    ESP_ERROR_CHECK(esp_event_handler_instance_register_with(view_event_handle, 
                                                            VIEW_EVENT_BASE, VIEW_EVENT_SENSOR_TDS_HISTORY, 
                                                            __view_event_handler, NULL, NULL));   
...
//////////////////////////////////////////////EC///////////////////////////////////////////////////////////////////////
    ...
    ESP_ERROR_CHECK(esp_event_handler_instance_register_with(view_event_handle, 
                                                            VIEW_EVENT_BASE, VIEW_EVENT_SENSOR_EC_HISTORY, 
                                                            __view_event_handler, NULL, NULL));   
...
// // Assuming you have a global variable to store the sensor data
...
//     while (1) {
//         enum pkt_type packetType = receivePacketType();
//         // ...

//         switch (packetType) {
//             case PKT_TYPE_SENSOR_SCD41_CO2:
//                     sensorTemperature = receivePacketType();
//                     break;
//             // case PKT_TYPE_SENSOR_SHT4x_TEMP:
//             // case PKT_TYPE_SENSOR_SHT4x_HUMIDITY:
//             case PKT_TYPE_SENSOR_TVOC_INDEX:
//                     sensorHumidity = receivePacketType();
//                     break;
//             // case PKT_TYPE_SENSOR_TDS:
//             // case PKT_TYPE_SENSOR_EC:
        

//                 // Add delay or other synchronization mechanisms if needed
//                 // delay(1000);
...
// float getCO2() { return sensorTemperature; }
// float gettVOC() { return sensorHumidity; }

// void DHT22_task_start(void) {
//     // Call the function to initialize the MQTT client
...
// Function to simulate reading sensor values from packet serial
void readPacketSerialSensor(void) {
    // Simulate reading packet from serial, replace this with actual code
    enum pkt_type packetType = (enum pkt_type)(rand() % 4 + 0xA0);  // Simulating receiving different packet types
    ...
    switch (packetType) {
        case PKT_TYPE_SENSOR_SCD41_CO2:
            // Read CO2 value
            float co2Value;
            // Replace the following line with actual code to read float value from serial
            // For example: uart_read_bytes(), sscanf(), etc.
            // sscanf(receivedData, "%f", &co2Value);
            printf("Received CO2 value: %.2f\n", co2Value);
            break;

        case PKT_TYPE_SENSOR_SHT4x_TEMP:
            // Read SHT4x temperature value
            float sht4xTempValue;
            // Replace the following line with actual code to read float value from serial
            // sscanf(receivedData, "%f", &sht4xTempValue);
            printf("Received SHT4x Temperature value: %.2f\n", sht4xTempValue);
            break;

        case PKT_TYPE_SENSOR_SHT4x_HUMIDITY:
            // Read SHT4x humidity value
            float sht4xHumidityValue;
            // Replace the following line with actual code to read float value from serial
            // sscanf(receivedData, "%f", &sht4xHumidityValue);
            printf("Received SHT4x Humidity value: %.2f\n", sht4xHumidityValue);
            break;

        case PKT_TYPE_SENSOR_TVOC_INDEX:
            // Read TVOC index value
            float tvocIndexValue;
            // Replace the following line with actual code to read float value from serial
            // sscanf(receivedData, "%f", &tvocIndexValue);
            printf("Received TVOC Index value: %.2f\n", tvocIndexValue);
            break;
...
// Define the task that reads sensor values from packet serial
static void PacketSerialSensor_task(void *pvParameter) {
    // Initialize any required resources or configurations for packet serial

    for (;;) {
        // Read sensor values from packet serial
        readPacketSerialSensor();

        // Add your code here to process or store the sensor values

        // Wait for a specific interval before reading again
        vTaskDelay(5000 / portTICK_PERIOD_MS);  // Adjust the delay as needed
    }
}


// Function to start the packet serial sensor task
void PacketSerialSensor_task_start(void) {
    xTaskCreatePinnedToCore(&PacketSerialSensor_task, "PacketSerialSensor_task", PACKET_SERIAL_TASK_STACK_SIZE, NULL, PACKET_SERIAL_TASK_PRIORITY, NULL, PACKET_SERIAL_TASK_CORE_ID);
}
